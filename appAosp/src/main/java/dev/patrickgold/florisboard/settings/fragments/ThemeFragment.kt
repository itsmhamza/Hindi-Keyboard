    ...
    override fun onCreateView(
        ...
        launch(Dispatchers.Default) {
            val themeContext = ContextThemeWrapper(
                context,
                FlorisBoard.getDayNightBaseThemeId(prefs.internal.themeCurrentIsNight)
            )
            ...
            keyboardView.apply {
                isPreviewMode = true
                computedLayout =
                    layoutManager.fetchComputedLayoutAsync(KeyboardMode.CHARACTERS, subtype, prefs).await()
                updateVisibility()

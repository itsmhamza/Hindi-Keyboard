...
//        }
        ...
        computedLayout.arrangement.forEach { keydataList ->
            val rowView = KeyboardRowView(context)
            keydataList.forEach {
                val keyView = KeyView(this, it)
                keyView.florisboard = florisboard
                rowView.addView(keyView)
            }
            addView(rowView)
    ...
    fun dismissActiveKeyViewReference() {
        activeKeyView?.onFlorisTouchEvent(
            MotionEvent.obtain(
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val desiredHeight = if (isSmartbarKeyboardView || isPreviewMode) {
            ...
            (florisboard?.inputView?.desiredTextKeyboardViewHeight ?: MeasureSpec.getSize(
                heightMeasureSpec
            ).toFloat())
        } * if (isPreviewMode) {
            0.90f

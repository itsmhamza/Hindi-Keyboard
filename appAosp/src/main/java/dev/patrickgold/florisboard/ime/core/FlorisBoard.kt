/*
 * Copyright (C) 2020 Patrick Go ldinger
 *
...
import dev.patrickgold.florisboard.ime.suggesstions.db.SuggesstionsDatabase
...
class FlorisBoard : InputMethodService(), ClipboardManager.OnPrimaryClipChangedListener {
    ...
    init {
        ...
        textInputManager = TextInputManager.getInstance(this)
        mediaInputManager = MediaInputManager.getInstance()
    }


    companion object {
        //private const val IME_ID: String = "com.quranreading.appyasincheck"
        private const val IME_ID: String = "com.quranreading.appyasincheck/.ime.core.FlorisBoard"
        private const val IME_ID_DEBUG: String =
            "dev.patrickgold.florisboard.debug/dev.patrickgold.florisboard.ime.core.FlorisBoard"
...
//        setupSuggestions(this)
        SuggesstionsDatabase.getInstance(this)
...
//                            .penaltyDeath()
    ...
    fun registerInputView(inputView: InputView) {
        ...
        inputView.toogle_remove_text_translator?.setOnClickListener {
            textInputManager.textTranslationSwitchOff()

        }
        inputView.translate_text?.setOnClickListener {
            textInputManager.translateTextCommit()
        }
    ...
    override fun onDestroy() {
        ...
        SuggesstionsDatabase.destroyInstance()
        super.onDestroy()
    }
...
//        }, 7)
    ...
    override fun onViewClicked(focusChanged: Boolean) {
        ...
        eventListeners.toList().forEach { it?.get()?.onViewClick(focusChanged) }
    }
...
//        adlayout =
//            inputWindowView?.findViewById(R.id.native_banner_ad_container)
//
//        val shimmerlayout =
//            inputWindowView?.findViewById<ShimmerFrameLayout>(R.id.native_banner_ad_shimmer)
//
//
////        val packageName: String = currentInputEditorInfo.packageName
////        if (!packageName.isNullOrEmpty() && isInternetOn())
////            when (packageName) {
...
////                "com.android.vending" -> adlayout?.visibility = View.GONE
////                else -> {
...
//    }
    ...
    fun setActiveInput(type: Int) {
        when (type) {
            R.id.text_input -> textInputManager.textViewGroup?.let { activeInputMainViewFlipper(it) }
            R.id.media_input -> mediaInputManager.mediaViewGroup?.let {
                activeInputMainViewFlipper(
                    it
                )
            }
            R.id.mic_input -> textInputManager.micLayout?.let { activeInputMainViewFlipper(it) }
        }
    }

    fun activeInputMainViewFlipper(view: View) {
        inputView?.mainViewFlipper?.displayedChild =
            inputView?.mainViewFlipper?.indexOfChild(view) ?: 0
    }
    ...
    interface EventListener {
        ...
        fun onViewClick(focusChanged: Boolean) {}
    }
    ...
    fun toggleTransletratiOnOff() {

        when (prefs.keyboard.transletration) {
            "off" -> {
                prefs.keyboard.transletration = "on"
                Toast.makeText(this, "Transliteration ON", Toast.LENGTH_SHORT).show()
            }
            else -> {
                prefs.keyboard.transletration = "off"
                Toast.makeText(this, "Transliteration OFF", Toast.LENGTH_SHORT).show()
            }
        }

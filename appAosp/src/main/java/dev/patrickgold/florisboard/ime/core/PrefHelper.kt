...
import dev.patrickgold.florisboard.ime.core.PrefHelper.Keyboard.Companion.TRANSLETRATION_TOGGLE
...
class PrefHelper(
    ...
    val translateLang = TranslationLang(this)
    ...
    class Gestures(private val prefHelper: PrefHelper) {
        ...
        var swipeVelocityThreshold: VelocityThreshold
            get() = VelocityThreshold.fromString(
                prefHelper.getPref(
        ...
        var swipeDistanceThreshold: DistanceThreshold
            get() = DistanceThreshold.fromString(
                prefHelper.getPref(
    ...
    class Keyboard(private val prefHelper: PrefHelper) {
        companion object {
            ...
            const val TRANSLATION = "keyboard__translation_enabled"
            const val TRANSLETRATION_TOGGLE ="keyboard_transletration"
            const val TEXT_TRANSLATOR ="keyboard_text_translator"
        }
        var texttranslator: String
            get() = prefHelper.getPref(TEXT_TRANSLATOR, "off")
            set(value) = prefHelper.setPref(TEXT_TRANSLATOR, value)
        var transletration: String
            get() = prefHelper.getPref(TRANSLETRATION_TOGGLE, "off")
            set(value) = prefHelper.setPref(TRANSLETRATION_TOGGLE, value)
        var translation: Boolean = false
            get() = prefHelper.getPref(TRANSLATION, false)
            private set
        ...
        var heightFactor: String
            ...
            set(v) = prefHelper.setPref(HEIGHT_FACTOR, v)
    ...
    class TranslationLang(private val prefHelper: PrefHelper) {
        companion object {
            const val INPUT_LANG_CODE = "input__lang"
            const val OUTPUT_LANG_CODE = "output__lang"
        }

        var inputLangCode: Int
            get() = prefHelper.getPref(INPUT_LANG_CODE, 57)
            set(v) = prefHelper.setPref(INPUT_LANG_CODE, v)

        var outputLangCode: Int
            get() = prefHelper.getPref(OUTPUT_LANG_CODE, 15)
            set(v) = prefHelper.setPref(OUTPUT_LANG_CODE, v)
    }
    /**
